...
struct InvariantTestRun<FEN: FoundryEvmNetwork> {
    ...
    line_coverage: Option<HitMaps>,
    // Source coverage staged until the run is accepted.
    source_coverage: Option<SourceHitMaps>,
        ...
        'stop: while should_continue_invariant_worker(campaign_state, runs, plan) {
            ...
            while current_run.depth < run_depth {
                ...
                SourceHitMaps::merge_opt(
                    &mut current_run.source_coverage,
                    call_result.source_coverage.take(),
                );
            ...
            invariant_test.merge_source_coverage(current_run.source_coverage.take());
