...
impl TestExecutionOptions {
    ...
    pub(crate) fn coverage(inline_config: Arc<InlineConfig>) -> Self {
        Self { coverage_mode: CoverageMode::Bytecode, ..Self::default_run(inline_config) }
    }

    pub(crate) fn source_coverage(inline_config: Arc<InlineConfig>) -> Self {
        Self { coverage_mode: CoverageMode::Source, ..Self::default_run(inline_config) }
    }
}
...
impl TestArgs {
    ...
    pub(crate) async fn run_tests(
        ...
        self.ensure_mutation_mode_compatible(execution.coverage_mode != CoverageMode::None)?;
    ...
    async fn build_and_run_tests<FEN: FoundryEvmNetwork>(
        ...
        let runner = MultiContractRunnerBuilder::new(config.clone(), execution.inline_config)
            ...
            .coverage_mode(execution.coverage_mode)
